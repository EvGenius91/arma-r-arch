...
| EntityService / [[EntityManager.md\|EntityManager]] | spawn, remove, реестр сущностей | не знает про корзины и цены |
| BankingService / [[BankingManager.md\|BankingManager]] | наличные и счета в мире | не знает про SellCard |
| EventBus (локальный, на игровом сервере) | уведомляет подписчиков после apply | не меняет мир сам |

Подробнее о командах: [[../CommandBus/Commands.md]]. О CommandBus: [[../CommandBus/CommandBus.md]].
...
CommandBus получает от бекенда императивные команды: «удали предмет с таким uid», «зачисли столько-то наличных». Игровой сервер обязан выполнить команду и вернуть отчёт: успех или причина отказа.

Воркер опрашивает бекенд раз в секунду ([[../CommandBus/CommandBus.md]]). Чтобы не ждать следующего цикла, можно вызвать `ForceRefresh()` — команды будут запрошены и выполнены сразу.
...
После того как команда успешно применена в мире, профильный сервис может опубликовать **локальное событие**. На него подписываются компоненты, которым нужна реакция, а не изменение мира:

- чат: «продан AK-74 за 500»;
- UI магазина: обновить инвентарь и баланс;
- оповещения: показать сообщение игроку.

Подписчики не знают про TradeManager и бекенд — так же, как описано для BankingManager в [[../Common concepts.md]]: менеджер рассылает событие, а компонент меню или оповещений сам решает, что с ним делать.

**CommandBus** доставляет команды и меняет мир. **EventBus** сообщает о том, что мир уже изменился.
...
- Предмет или техника **появляются в мире** через CommandBus: `addNewEntityToCharacterInventory`, `spawnNewVehicleToNearSlot` ([[../CommandBus/Commands.md]]).
