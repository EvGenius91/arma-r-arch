...
| CommandBus (`removeEntity` и т.п.) | обычные мутации бекенд → мир (продажа, админ); не путать с анализатором |

Команда `removeEntity` с бекенда по-прежнему выполняется даже для локально заблокированной сущности; hard-reset анализатора — отдельный сценарий сверки дублей.
